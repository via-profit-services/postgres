...
const provider: PostgresProvider = async ({ config, emitter }) => {
  ...
  state.pool.on('error', (err, client) => emitter.emit('postgres-pool-error', err, client));

  try {
    const client = await state.pool.connect();
    client.on('error', err => emitter.emit('postgres-error', err));
    client.on('end', () => emitter.emit('postgres-end'));
    client.on('notice', notice => emitter.emit('postgres-notice', notice));
